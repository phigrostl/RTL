...
namespace RTL {
    ...
    Renderer::BoundingBox Renderer::GetBoundingBox(const Vec4(&fragCoords)[3],
        ...
        minX = (float)Clamp(minX, 0.0f, (float)(width - 1));
        maxX = (float)Clamp(maxX, 0.0f, (float)(width - 1));
        minY = (float)Clamp(minY, 0.0f, (float)(height - 1));
        maxY = (float)Clamp(maxY, 0.0f, (float)(height - 1));
