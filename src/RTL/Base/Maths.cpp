...
namespace RTL {
    ...
    */
    ...
    std::vector<float> getNumbersFromString(std::string str) {
        ...
        for (size_t i = 0; i < str.size(); i++) {
            if (str[i] >= '0' && str[i] <= '9') {
                if (i == 0)
                ...
                else if (isNumber(str[i - 1]) || str[i - 1] == '.' || str[i - 1] == '-') {
                    if (isFloat == -1)
                    {
                        if (isNegative)
                    ...
                    {
                        if (isNegative)
                            numbers.back() = numbers.back() - (str[i] - '0') / (float)pow(10, isFloat);
                        else
                            numbers.back() = numbers.back() + (str[i] - '0') / (float)pow(10, isFloat);
                        isFloat++;
                    }
    ...
    float getTriangleArea(const Vec2& p0, const Vec2& p1, const Vec2& p2) {
        return fabs((p0.X * (p1.Y - p2.Y) + p1.X * (p2.Y - p0.Y) + p2.X * (p0.Y - p1.Y)) / 2.0f);
    }

    float getPointInTriangleDepth(Vec3 d1, Vec3 d2, Vec3 d3, Vec2 p) {
        float area = getTriangleArea(d1, d2, d3);
        float area1 = getTriangleArea(p, d2, d3);
        float area2 = getTriangleArea(d1, p, d3);
        float area3 = getTriangleArea(d1, d2, p);
        Vec3 depths = Vec3(1 / d1.Z, 1 / d2.Z, 1 / d3.Z);
        Vec3 weights = Vec3(area1, area2, area3);
        float depth = Dot(depths, weights);
        return 1 / (depth / area);
    }

    float TanToFov(float tan, Vec2 size) {
        return 2 * (float)atan(sqrt(size.X * size.X + size.Y * size.Y) / (2 * (double)tan));
    }

    float FovToTan(float fov, Vec2 size) {
        return (float)(sqrt(size.X * size.X + size.X * size.X)) / (float)(2 * (double)tan(fov / 2));
    }
